...
![Version: 0.4.7](https://img.shields.io/badge/Version-0.4.7-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: 1.0.78426](https://img.shields.io/badge/AppVersion-1.0.78426-informational?style=flat-square)
...
| global.ingress | object | `{"className":"harness","defaultbackend":{"create":false,"image":{"digest":"","pullPolicy":"IfNotPresent","registry":"registry.k8s.io","repository":"defaultbackend-amd64","tag":"1.5"}},"enabled":false,"hosts":[""],"loadBalancerEnabled":false,"loadBalancerIP":"0.0.0.0","nginx":{"controller":{"annotations":{}},"create":false,"image":{"digest":"","pullPolicy":"IfNotPresent","registry":"us.gcr.io","repository":"k8s-artifacts-prod/ingress-nginx/controller","tag":"v1.0.0-alpha.2"},"objects":{"annotations":{}}},"tls":{"enabled":false,"secretName":"harness-cert"}}` | - Set `ingress.enabled` to `true` to create Kubernetes *Ingress* objects for Nginx. |
...
| global.monitoring | object | `{"enabled":false,"prometheus":{"external":true,"hostIp":""}}` | Enable monitoring for harness databases |
| global.monitoring.prometheus.external | bool | `true` | if false, then its assumed that prometheus is running in the same cluster as harness and then no ingress/virtualservice would be exposed for metrics endpoints |
| global.monitoring.prometheus.hostIp | string | `""` | prometheus public IP if prometheus is hosted outside harness cluster. This will be whitelisted in ingress/virtualservice rules |
...
| platform.harness-manager | object | `{"affinity":{},"immutable_delegate_docker_image":{"image":{"tag":"23.03.78314"}},"nodeSelector":{},"tolerations":[]}` | harness-manager (taints, tolerations, and so on) |
